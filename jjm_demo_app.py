...
# UPDATED: SO name = "ROKI RAY" and Assamese Jalmitra names used instead of JM-1, JM-2...
...
def generate_demo_data(total_schemes: int = 20, so_name: str = "ROKI RAY"):
    ...
    - Maps Assamese Jalmitras to schemes,
    ...
    jalmitras = random.sample(assamese_names * 3, total_schemes)  # repeat list if needed
...
with col_gen:
    ...
    if st.button("Generate Demo Data") and not st.session_state.get("generating", False):
        ...
        st.session_state["generating"] = False
        st.success("✅ Demo data generated for SO: ROKI RAY (Assamese Jalmitras).")
...
merged_today = readings_df.merge(schemes_df[['id','scheme_name','functionality','so_name']],
                                 left_on='scheme_id', right_on='id', how='left') if not readings_df.empty else pd.DataFrame()
...
if last7_all.empty:
    ...
    metrics_df['score'] = metrics_df['days_norm'] * 0.5 + metrics_df['qty_norm'] * 0.5
    ...
    top_table = metrics_df.sort_values(by='score', ascending=False).head(10)[['Rank','jalmitra','days_updated','total_water_m3','score']]
    worst_table = metrics_df.sort_values(by='score', ascending=True).head(10)[['Rank','jalmitra','days_updated','total_water_m3','score']]

    def style_top(df): return df.style.background_gradient(cmap='Greens', subset=['score'])
    def style_worst(df): return df.style.background_gradient(cmap='Reds_r', subset=['score'])
...
with st.expander("ℹ️ How ranking is computed"):
    st.markdown("""
    - Days Updated (last 7d)
    - Total Water (m³)
    - Score = 0.5 × Days Updated + 0.5 × Water Quantity (normalized)
    """)

st.success(f"Dashboard ready for SO: ROKI RAY | Demo data: {st.session_state.get('demo_generated', False)} ✅")
